...
async fn index() -> impl Responder {
    info!("index route");
    HttpResponse::Ok().append_header(header::ContentType::html()).body(r#"<!doctype html>
<html lang=en-US>
<meta charset=utf8>
<body>
    ...
    <div><h2>POST /labelize</h2>
        ...
        <div>By default, the prefered languages of labels used are, in order: Japanese (Z1830), Chinese (Z1006), English (Z1002). This default value is to test and show that the language switching function works.</div>
    ...
    <div><h2>POST /compacify</h2>
        ...
        <div>The main transformation we do is that we "raise" the type (Z1K1) of ZObjects (all ZObjects has its type in the key Z1K1) and the type in Arrays (all Arrays have the type as the first element) upwards, into the parent object's key.</div>
        <div>In other words, we separate the type information from the rest of the data. The type information is merged into the key of objects instead.</div>
        <div>Similarly, if an objects only have one key-value pair, the key is merged upward.</div>
    ...
    <div><h2>Notes</h2>
        ...
        <div><h3>Processing time</h3>
            <div>The tool needs to fetch info about all the ZIDs mentioned in the request payload from wikifunction. Depending on the size of request, fetching all these information may take 10 seconds or more.</div>
            <div>Fetched results are cached for 10 minutes, so there may be a delay between changes in wikifunction, and results returned from this tool.</div>
        </div>
        <div><h3>Feedback wanted</h3>
            <div>This tool is still in active development (2022-09-05)</div>
            <div>Please do contact me and provide feedback for:<ul>
